error id: file://<WORKSPACE>/data/code-rep-dataset/Dataset5/Tasks/8281.java
file://<WORKSPACE>/data/code-rep-dataset/Dataset5/Tasks/8281.java
...
```java
offset: 1
uri: file://<WORKSPACE>/data/code-rep-dataset/Dataset5/Tasks/8281.java
text:
```scala
i@@nt flags = ImageList.COLOR_FLAGS;
	...
	for (int i=0; i<items.length; i++) {
		...
		if (item != null) {
			...
			if (strings != null) {
				...
				System.arraycopy (strings, index, temp, index + 1, count - index);
				item.strings = temp;
			}
			Image [] images = item.images;
			if (images != null) {
				Image [] temp = new Image [count + 1];
				System.arraycopy (images, 0, temp, 0, index);
				System.arraycopy (images, index, temp, index + 1, count - index);
				item.images = temp;
			}
			if (index == 0) {
				if (count != 0) {
					if (strings == null) {
						item.strings = new String [count + 1];
						item.strings [1] = item.text;
					}
					item.text = "";
					if (images == null) {
						item.images = new Image [count + 1];
			...
			if (item.cellBackground != null) {
				...
				System.arraycopy (cellBackground, index, temp, index + 1, count - index);
				temp [index] = -1;
			...
			if (item.cellForeground != null) {
				...
				System.arraycopy (cellForeground, index, temp, index + 1, count - index);
				temp [index] = -1;
			...
			if (item.cellFont != null) {
				...
				System.arraycopy (cellFont, index, temp, index + 1, count- index);
				temp [index] = -1;
				item.cellFont = temp;
			}
		}
...
QDox parse error in file://<WORKSPACE>/data/code-rep-dataset/Dataset5/Tasks/8281.java
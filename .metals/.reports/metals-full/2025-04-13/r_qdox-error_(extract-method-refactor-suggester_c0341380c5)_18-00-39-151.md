error id: file://<WORKSPACE>/data/code-rep-dataset/Dataset5/Tasks/3232.java
file://<WORKSPACE>/data/code-rep-dataset/Dataset5/Tasks/3232.java
...
```java
offset: 8
uri: file://<WORKSPACE>/data/code-rep-dataset/Dataset5/Tasks/3232.java
text:
```scala
...
public class XMPPClientSOContainer extends ClientSOContainer implements
	...
	public void sendMessage(ID target, String message) throws IOException {
		getConnectionOrThrow().sendMessage(target, message);
	}

	protected ECFConnection getConnectionOrThrow() throws IOException {
		ECFConnection conn = (ECFConnection) getConnection();
	...
	protected void sendPresenceUpdate(ID target, Presence presence)
			throws IOException {
		getConnectionOrThrow().sendPresenceUpdate(target, presence);
	}

	protected void sendRosterAdd(String user, String name, String[] groups)
			throws IOException {
		getConnectionOrThrow().sendRosterAdd(user, name, groups);
	}

	protected void sendRosterRemove(String user) throws IOException {
		getConnectionOrThrow().sendRosterRemove(user);
	}
...
QDox parse error in file://<WORKSPACE>/data/code-rep-dataset/Dataset5/Tasks/3232.java
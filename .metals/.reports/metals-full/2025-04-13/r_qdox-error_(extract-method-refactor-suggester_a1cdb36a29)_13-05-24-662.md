error id: file://<WORKSPACE>/data/code-rep-dataset/Dataset4/Tasks/9339.java
file://<WORKSPACE>/data/code-rep-dataset/Dataset4/Tasks/9339.java
...
```java
offset: 1
uri: file://<WORKSPACE>/data/code-rep-dataset/Dataset4/Tasks/9339.java
text:
```scala
...
public class Skin implements Disposable {
	...
	static private Method findMethod (Class type, String name) {
		Method[] methods = ReflectionCache.getType(type).getMethods();
	...
	public void dispose () {
		...
		for (Entry<Class, ObjectMap<String, Object>> entry : resources.entries()) {
			if (!ReflectionCache.getType(entry.key).isAssignableFrom(ReflectionCache.getType(Disposable.class))) continue;
	...
	protected Json getJsonLoader (final FileHandle skinFile) {
		...
		class AliasWriter implements Serializer {
			...
			public void write (Json json, Object object, Class valueType) {
				...
				throw new SerializationException(object.getClass().getName() + " not found: " + object);
			}
		...
		json.setSerializer(Skin.class, new Serializer<Skin>() {
			...
			private void readTypeMap (Json json, ObjectMap<String, ObjectMap> typeToValueMap, boolean isResource) {
				...
				for (Entry<String, ObjectMap> typeEntry : typeToValueMap.entries()) {
					...
					try {
						readNamedObjects(json, ReflectionCache.forName(className).getClassOfType(), valueMap, isResource);
...
QDox parse error in file://<WORKSPACE>/data/code-rep-dataset/Dataset4/Tasks/9339.java